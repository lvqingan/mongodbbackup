...
for KEY in "${!DBHOST[@]}"; do
  ...
  if [ -n "${DBCOLLECTIONS[$KEY]}" ]; then
    if  [ ${DBCOLLECTIONSMATCH[$KEY]} = "exclude" ]; then
      COLLECTIONS=''
      for collection in ${DBCOLLECTIONS[$KEY]}; do
  ...
  for database in $DBS; do
    ...
    mongodump --host ${DBHOST[$KEY]} --port ${DBPORT[$KEY]} --username=${DBUSER[$KEY]} --password=${DBPASS[$KEY]} \
      ${DBOPTIONS[$KEY]} --db $database $COLLECTIONS > $BACKDIR/$SERVER-$database-$DATE-mongodbbackup.sql
    $COMPRESSION_COMMAND $BACKDIR/$SERVER-$database-$DATE-mongodbbackup.sql
    echo "done!"
  done
done
...
if  [ $MAIL = "y" ]; then
  ...
  BODY=$BODY`cd $BACKDIR; for file in *$DATE-mongodbbackup.sql.$COMPRESSION_EXTENSION; do md5sum ${file};  done`
  ATTACH=`for file in $BACKDIR/*$DATE-mongodbbackup.sql.$COMPRESSION_EXTENSION; do echo -n "-a ${file} ";  done`
...
if  [ $DELETE = "y" ]; then
  OLDDBS=`cd $BACKDIR; find . -name "*-mongodbbackup.sql.$COMPRESSION_EXTENSION" -mtime +$DAYS`
...
if  [ $FTP = "y" ]; then
  ...
  cd $BACKDIR
  ATTACH=`for file in *$DATE-mongodbbackup.sql.$COMPRESSION_EXTENSION; do echo -n -e "put ${file}\n"; done`
